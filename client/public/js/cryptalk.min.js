(function () {
    ...
    function on(fromId, event, handler, once) {
        let i,
            item,
            subscription = [fromId, handler, handler.length > 1];
        (subs[event] || (subs[event] = [])).push(subscription);
        if (emits[event]) {
            i = 0;
            subscription = [subscription];
            while ((item = emits[event][i++])) {
                emit(0, 0, item[0], item[1], subscription);
                if (once) {
                    break;
                }
            }
    ...
    function notifications(mediator, settings, win) {
        var enabled = true,
            ...
            doBlink = function () {
                if (enabled) {
                    if (win.getTitle() === original_title) win.setTitle(new_title);
                    else win.setTitle(original_title);
            ...
            notify = function (title, body, icon, fallback) {
                if (enabled && now() - last > settings.notifications.maxOnePerMs) {
                    ...
                    if (native_supported && Notification.permission === "granted") {
                        ...
                        blinkTitleUntilFocus("Attention", 1e3);
                    }
                }
            };
        native_supported = window.Notification !== undefined;
        mediator.on("notification:send", function (data) {
            notify(data.title, data.body, data.icon, true);
        });
        mediator.on("notification:on", function () {
            on();
        });
        mediator.on("notification:off", function () {
            off();
        });
        enableNative();
        off();
    ...
    var templates = {
        help:
            "<pre>                                                                \n" +
            "Fargochat, chat secreto.                                      \n" +
            "                                                                       \n" +
            "----------------------------------------------------------------------\t\n" +
            "                                                                       \n" +
            "Cliente:                                                    \t\t\t\n" +
            "\t--clave\t\tClave\tMete una clave                 \n" +
            "\t--nick\t\tNombree\t\tSets an optional nick                   \n" +
            "\t--mute  \t\t\t\t\tAudio on\t\t\t\t\t\t\t\t\t\n" +
            "\t--unmute  \t\t\t\tAudio off\t\t\t\t\t\t\t\t\t\n" +
        ...
        client: { title: "FargoChat - Offline" },
    };
    var settings = {
    ...
    var exports$2 = {},
        reDigits = /^\d+$/;
    exports$2.io = io;
    exports$2.SHA1 = function (string) {
        return cryptoJs.exports.SHA1(string).toString();
    };
    exports$2.AES = {
        decrypt: function (string, fgh) {
            return cryptoJs.exports.AES.decrypt(string, fgh).toString(cryptoJs.exports.enc.Utf8);
        },
        encrypt: function (string, fgh) {
            return cryptoJs.exports.AES.encrypt(string, fgh).toString();
        },
    };
    exports$2.ssplit = function (string, seperator) {
        var components = string.split(seperator);
        return [components.shift(), components.join(seperator)];
    };
    exports$2.activeElement = function () {
        try {
    ...
    exports$2.isDigits = function (value) {
        return reDigits.test(value);
    };
    exports$2.template = function (str, map) {
        return (
            str &&
            str.replace(/{(\w+)}/gi, function (outer, inner) {
                return Object.prototype.hasOwnProperty.call(map, inner) ? map[inner] : outer;
            })
        );
    };
    exports$2.getJSON = function (path, onSuccess, onError) {
    ...
    var exports$1 = {};
    exports$1.html = function (string) {
        this.forEach(function (element) {
            element.innerHTML = string;
        });
        return this;
    };
    exports$1.append = function (string) {
        this.forEach(function (element) {
            element.innerHTML += string;
        });
        return this;
    };
    exports$1.first = function () {
        return this[0];
    };
    exports$1.on = function (eventName, callback) {
        this.forEach(function (element) {
    ...
    for (var k in exports$1) ElementArray.prototype[k] = exports$1[k];
    ...
    for (var l in exports$2) Dollar[l] = exports$2[l];
    function host(mediator, settings, templates) {
        var socket,
            ...
            connect = function (toHost, done) {
                ...
                if (host && host.connected) {
                    mediator.emit("console:error", Dollar.template(templates.messages.already_connected, { host: host.host }));
                    mediator.emit("console:unlockInput");
                    return;
                }
                mediator.emit("console:info", Dollar.template(templates.messages.connecting, { host: host.host }));
                mediator.emit("console:motd", settings.motd);
                socket = Dollar.io(host.host, { forceNew: true, "force new connection": true });
                socket
                    .on("room:joined", function () {
                        mediator.emit("console:info", Dollar.template(templates.messages.joined_room, { roomName: Dollar.escapeHtml(parameters.room) }));
                        socket.emit("room:count");
                    })
                    .on("room:left", function () {
                        mediator.emit("console:info", Dollar.template(templates.messages.left_room, { roomName: Dollar.escapeHtml(parameters.room) }));
                        mediator.emit("room:changed", false);
                    })
                    .on("message:send", function (data) {
                        var decrypted = Dollar.AES.decrypt(data.msg, Dollar.SHA1(parameters.room) + parameters.key),
                            sanitized = Dollar.escapeHtml(decrypted),
                            nick = !data.nick ? templates.default_nick : Dollar.escapeHtml(Dollar.AES.decrypt(data.nick, Dollar.SHA1(parameters.room) + parameters.key));
                        if (!decrypted) {
                    ...
                    .on("message:server", function (data) {
                        if (data.msg) {
                            var sanitized = Dollar.escapeHtml(data.msg);
                            if (templates.server[sanitized]) {
                                if (data.payload !== undefined) {
                                    var sanitized_payload = Dollar.escapeHtml(data.payload);
                                    mediator.emit("console:server", Dollar.template(templates.server[sanitized], { payload: sanitized_payload }));
                                } else {
                    ...
                    .on("connect", function () {
                        mediator.emit("console:info", Dollar.template(templates.messages.connected, { host: host.host }));
                    ...
                    .on("disconnect", function () {
                        ...
                        mediator.emit("console:info", Dollar.template(templates.messages.disconnected, { host: host.host }));
    ...
    function client(mediator, settings, templates) {
        var nick,
            key,
            ...
            help = function () {
                mediator.emit("console:motd", templates.help);
            },
            clear = function () {
                mediator.emit("console:clear");
            },
            setTorch = function (payload) {
                mediator.emit("console:torch", payload);
            },
            setNick = function (payload) {
                if (payload.length > settings.nick.maxLen) {
                    ...
                    return mediator.emit("console:error", Dollar.template(templates.messages.nick_to_short, { nick_minLen: settings.nick.minLen }));
                }
                nick = payload;
                mediator.emit("nick:changed", nick);
                mediator.emit("console:info", Dollar.template(templates.messages.nick_set, { nick: Dollar.escapeHtml(nick) }));
            },
            title = function (payload) {
                mediator.emit("window:title", payload);
                mediator.emit("console:info", Dollar.template(templates.messages.title_set, { title: Dollar.escapeHtml(payload) }));
            };
    ...
    function cons(mediator, settings, templates, sounds) {
        var components = { chat: Dollar("#chat"), input: Dollar("#input"), inputWrapper: Dollar("#input_wrapper") },
            parameters = {},
            commands = {
                post: function (type, text, nick) {
                    var tpl = templates.post[type],
                        uniqueId = "msg_" + new Date().getTime() + "_" + Math.round(Math.random() * 1e6),
                        post,
                        data = Object.assign({}, settings, { nick: nick, timestamp: new Date().toLocaleTimeString(), id: uniqueId });
                    data.text = Dollar.template(text, data);
                    post = Dollar.template(tpl, data);
                    commands.showNotification(type, nick, text);
                    setTimeout(function () {
                        var parent = components.chat.first(),
                            child = Dollar("#" + uniqueId).first();
                ...
                torch: function (ttl) {
                    ttl = parseInt(ttl, 10);
                    if (ttl > 0 && ttl < 3600) {
                        mediator.emit("console:info", Dollar.template(templates.messages.torch_is_now, { ttl: ttl }));
                        settings.ttl = ttl * 1e3;
                    } else {
                        mediator.emit("console:error", Dollar.template(templates.messages.torch_not_set));
                    }
                },
                param: function (p) {
                    parameters = Object.assign({}, parameters, p);
                ...
                showNotification: function (type, nick, text) {
                    var title = type !== "message" ? "FargoChat" : nick,
                        icon = type === "message" ? "gfx/icon_128x128.png" : type === "error" ? "gfx/icon_128x128_error.png" : "gfx/icon_128x128_info.png";
                    mediator.emit("notification:send", { title: title.substring(0, 20), body: text.substring(0, 80), icon: icon });
                    if (type === "message") {
            ...
            onKeyDown = function (e) {
                var buffer, parts, payload, command;
                if (!e.ctrlKey && !e.altKey && components.input[0] !== Dollar.activeElement()) {
                    components.input.focus();
                    return;
                }
                ...
                if ((buffer[0] || buffer.slice(0, 1)) === "/") {
                    parts = Dollar.ssplit(buffer.slice(1), " ");
                    command = parts[0];
                    payload = parts[1];
                    mediator.emit("command:" + command, payload, function (retvals, recipients) {
                        if (!recipients) {
                            commands.post("error", Dollar.template(templates.messages.unrecognized_command, { commandName: command }));
                            return;
                        } else {
                            commands.clearInput();
                        }
                    ...
                    mediator.emit("socket:emit", {
                        data: "message:send",
                        payload: {
                            room: Dollar.SHA1(parameters.room),
                            msg: Dollar.AES.encrypt(buffer, Dollar.SHA1(parameters.room) + parameters.key).toString(),
                            nick: parameters.nick ? Dollar.AES.encrypt(parameters.nick, Dollar.SHA1(parameters.room) + parameters.key).toString() : false,
                        },
                    });
                    commands.clearInput();
                }
            };
        Dollar(document).on("keydown", onKeyDown);
        components.input.focus();
    ...
    function room(mediator, settings, templates) {
        var room = false,
            join = function (payload) {
                if (room !== false) {
                    mediator.emit("console:error", Dollar.template(templates.messages.already_in_room, { room: room }));
                } else if (payload.length >= settings.room.maxLen) {
                    mediator.emit("console:error", Dollar.template(templates.messages.room_name_too_long));
                } else if (payload.length < settings.room.minLen) {
                    mediator.emit("console:error", Dollar.template(templates.messages.room_name_too_short));
                } else {
                    room = payload;
                    mediator.emit("room:changed", room).emit("socket:emit", { data: "room:join", payload: Dollar.SHA1(room) });
                }
            },
            leave = function () {
                if (room !== false) {
                    mediator.emit("socket:emit", { data: "room:leave", payload: Dollar.SHA1(room) });
                    room = false;
                } else {
    ...
    let wind = win(castrato);
    cons(castrato, settings, templates, sounds);
    notifications(castrato, settings, wind);
    client(castrato, settings, templates);
    host(castrato, settings, templates);
    room(castrato, settings, templates);
    castrato
        .on("window:focused", function () {
            castrato.emit("audio:off");
            castrato.emit("notification:off");
        })
        .on("window:blurred", function () {
            castrato.emit("audio:on");
            castrato.emit("notification:on");
        })
        .on("command:mute", function () {
            castrato.emit("audio:mute");
        })
        .on("command:unmute", function () {
            castrato.emit("audio:unmute");
        })
        .on("room:changed", function (room) {
            castrato.emit("console:param", { room: room }).emit("host:param", { room: room });
        })
        .on("nick:changed", function (nick) {
            castrato.emit("console:param", { nick: nick });
        })
        .on("key:changed", function (key) {
            castrato.emit("console:param", { key: key }).emit("host:param", { key: key });
        });
    castrato.emit("command:connect", undefined, function () {
        var hash = window.location.hash;
        if (hash) {
            var parts = hash.slice(1).split(":");
            if (parts[0]) {
                castrato.emit("command:join", parts[0]);
            }
            if (parts[1]) {
                castrato.emit("command:key", parts[1]);
            }
        }
    });
})();
